    ...
    public class FormUltils
    {
        ...
        {            
            ...
            try
            {
                ...
                worksheet.Range[worksheet.Cells[cellRowIndex, cellColumnIndex], worksheet.Cells[cellRowIndex, numberOfTableColumn]].Font.Size = 14;
                ...
                if(second != null && second.Keys.Count > 0)
                {
                    ...
                    foreach (KeyValuePair<string, string> entry in second)
                    {
                        ...
                        if (index > 0 && index % 2 == 0)
                        {
                            worksheet.Cells[cellRowIndex2, distance + index + 1] = entry.Key;
                            worksheet.Cells[cellRowIndex2, distance + index + 2] = entry.Value;
                        }
                        else
                        {
                            worksheet.Cells[cellRowIndex2, index + 2] = entry.Key;
                            worksheet.Cells[cellRowIndex2, index + 3] = entry.Value;
                        }
                ...
                worksheet.Cells[cellRowIndex + 2, numberOfTableColumn - 1] = StringHeaderReports.LAST_MILE;
                ...
                logPath = Environment.CurrentDirectory + @"\Reports";// +"_" + DateTime.Now.ToString("HH");
                if (!Directory.Exists(logPath))
                {
                    Directory.CreateDirectory(logPath);
                }
                string fileNameExcel = logPath + @"\" + fileName + DateTime.Now.ToString("ddMMyyyHHmmss") + ".xls";
                workbook.SaveAs(fileNameExcel);
                excel.Quit();
                workbook = null;
                excel = null;
                Process.Start("EXCEL.EXE", "\"" + fileNameExcel + "\"");
            }
